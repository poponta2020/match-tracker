...
vi.mock('../api', () => ({
  ...
  matchVideoAPI: {
    getDateCandidates: mocks.videoGetDateCandidates,
...
describe('VideoRegisterModal - 選手起点（PlayerSourceSelect）', () => {
  beforeEach(() => {
    ...
    mocks.videoGetDateCandidates.mockResolvedValue({ data: [] });
...
describe('VideoRegisterModal - 日付起点（DateSourceSelect / 日付別候補API）', () => {
  beforeEach(() => {
    Object.values(mocks).forEach((m) => m.mockReset());
    mocks.matchGetByPlayerId.mockResolvedValue({ data: [] });
    mocks.pairingGetByPlayerId.mockResolvedValue({ data: [] });
  ...
  it('日付起点は getDateCandidates(date) を呼び、返った候補を表示する。registered:true はグレーアウト（選択不可）', async () => {
    // サーバ側で統合・登録済み判定済みの候補を返す（参加日スコープなし）。
    // 第1試合は登録済み、第2試合は未登録。
    mocks.videoGetDateCandidates.mockResolvedValue({
      data: [
        {
          matchDate: '2026-06-12',
          matchNumber: 1,
    ...
    // 日付起点はデフォルトタブ。初期日付（ローカル）で候補APIが呼ばれる
    await waitFor(() => {
      expect(mocks.videoGetDateCandidates).toHaveBeenCalledWith(todayLocalISODate());
    });
    // 旧3-wayAPIは使われない
    expect(mocks.pairingGetByPlayerId).not.toHaveBeenCalled();
  ...
  it('非参加ユーザーでも候補が出る（候補APIは参加日スコープなし＝返れば表示される）', async () => {
    // 旧 getByDate は非参加者に空配列を返していたが、新APIはスコープが無いため
    // サーバが候補を返せばそのまま表示される（撮影担当者・第三者登録者を想定）。
    mocks.videoGetDateCandidates.mockResolvedValue({
      data: [
        {
          matchDate: '2026-06-12',
    ...
    // 自分が参加していない日でも、候補がそのまま選択可能な状態で表示される
    const card = await screen.findByText(/渡辺四郎/);
    expect(card.closest('button')).toBeTruthy();
  });

  it('日付起点で選択可能な候補をクリックすると register に自然キーが渡る', async () => {
    const user = userEvent.setup();
    mocks.videoGetDateCandidates.mockResolvedValue({
      data: [
        {
  ...
  it('日付起点で相手がシステム未登録（生IDが player2Id:0）の候補は「相手未登録」で選択不可になる', async () => {
    // 試合詳細・選手起点と統一: 未登録相手を含む試合は登録不可（選択不可）。
    // 判定は DTO の生選手IDで行う（round-4 のガード維持）。
    mocks.videoGetDateCandidates.mockResolvedValue({
      data: [
        {
          matchDate: '2026-06-12',
  ...
  it('日付起点の初期日付はローカル日付になり、その日付で候補取得が発火する（JST 早朝でも前日にならない）', async () => {
    // 初期日付は todayLocalISODate()（ローカルTZ基準）であるべき。
    // 旧実装 new Date().toISOString().split("T")[0] は UTC 基準のため、ローカルが
    // UTC+ のとき早朝に前日になりうる。JST 早朝での前日ズレ回避自体は date.test.js の
    // 純関数テストで検証済み。ここでは初期日付として実際にローカル日付が使われ、
    // その日付で候補取得が発火することを確認する（フェイクタイマーは waitFor と
    // 干渉するため使わず、実時刻のローカル日付と突き合わせる）。
    const expected = todayLocalISODate();

    const { container } = render(
      <VideoRegisterModal selectMode onClose={vi.fn()} onSuccess={vi.fn()} />
    );
    ...
    // その日付で候補取得（新API）が走っている（= 初期日付で fetch されている）
    await waitFor(() => {
      expect(mocks.videoGetDateCandidates).toHaveBeenCalledWith(expected);
    });
  });
});
