...
/** 選手用通知種別 */
const PLAYER_NOTIFICATION_TYPES = [
...
const ADMIN_NOTIFICATION_TYPES = [
  { key: 'adminWaitlistUpdate', label: 'キャンセル待ち状況通知' },
  { key: 'adminSameDayConfirmation', label: '当日確認まとめ' },
];

/**
 * LINEセクションパネル（1つの用途分のUI）
 */
const LineSectionPanel = ({
  title,
  channelType,
  playerId,
  notificationTypes,
  ...
  const fetchStatus = useCallback(async () => {
    try {
      setLoading(true);
      const res = await lineAPI.getStatus(playerId, channelType);
      setStatus(res.data);
      if (res.data.friendAddUrl) {
        setFriendAddUrl(res.data.friendAddUrl);
      }
    } catch (err) {
      console.error(`${title}の状態取得に失敗:`, err);
      setError(`${title}の状態取得に失敗しました`);
    } finally {
      setLoading(false);
    }
  }, [playerId, channelType, title]);

  useEffect(() => {
    if (playerId) fetchStatus();
  ...
  const handleEnable = async () => {
    try {
      ...
      setError(null);
      setMessage(null);
      const res = await lineAPI.enable(playerId, channelType);
  ...
  const handleDisable = async () => {
    ...
    try {
      ...
      setError(null);
      setMessage(null);
      await lineAPI.disable(playerId, channelType);
      setLinkingCode(null);
  ...
  const handleReissueCode = async () => {
    try {
      ...
      setError(null);
      setMessage(null);
      const res = await lineAPI.reissueCode(playerId, channelType);
      setLinkingCode(res.data.linkingCode);
      setCodeExpiresAt(res.data.codeExpiresAt);
  ...
  const displayMessage = message || globalMessage;
    ...
    <div className="space-y-4">
      ...
      {displayMessage && (
        <div className="bg-green-50 border border-green-200 rounded-lg p-3 flex items-center gap-2">
          ...
          <p className="text-green-800 text-sm">{displayMessage}</p>
        </div>
      )}

      {/* 連携状態 */}
      <div className="bg-white rounded-lg border p-4 space-y-4">
        <h3 className="font-semibold text-gray-700">連携状態</h3>

        {!status?.enabled ? (
          <div className="space-y-3">
            <p className="text-sm text-gray-600">
              LINE通知を有効にすると、通知をLINEで受け取れます。
            </p>
            <button
      ...
      {/* 通知種別ごとのON/OFF */}
      {status?.enabled && preferences && notificationTypes.length > 0 && (
        <div className="bg-white rounded-lg border p-4 space-y-3">
          <h3 className="font-semibold text-gray-700">通知種別</h3>
...
const LineSettings = () => {
  ...
  return (
    <div className="max-w-lg mx-auto p-4 space-y-6">
      ...
      {/* 選手用LINE */}
      <LineSectionPanel
        title={showAdminSection ? '選手用LINE' : 'LINE連携'}
        channelType="PLAYER"
        playerId={playerId}
        notificationTypes={PLAYER_NOTIFICATION_TYPES}
        preferences={preferences}
        onPreferenceChange={handleTogglePreference}
      />
