...
describe('MatchDetail - 取り札・お手付き詳細の読み取り専用表示', () => {
  ...
  it('C-14: メンター閲覧（?playerId=）では getCardRecord を呼ばず何も表示しない', async () => {
    ...
    // 読み込み完了（match 取得）まで待つ。card-record 取得 effect は match 依存で
    // このタイミングまでに評価済みのため、以降 getCardRecord 未呼び出しを確定検証できる。
    await waitFor(() => expect(matchAPI.getById).toHaveBeenCalled());
