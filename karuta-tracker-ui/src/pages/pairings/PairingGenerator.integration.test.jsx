...
describe('ロック以外を再シャッフル（reshuffle 導線 UI 側・AC-1〜7）', () => {
  describe('ボタン表示・文言（実関数を直接検証）', () => {
    it('組み合わせありの編集可能状態で表示される（AC-1）', () => {
      expect(shouldShowReshuffleButton({
        isReadOnly: false, isViewMode: false, pairings: [{ player1Id: 1, player2Id: 2 }],
      })).toBe(true);
    });
    it('閲覧モード・閲覧専用では非表示（AC-1）', () => {
      expect(shouldShowReshuffleButton({ isReadOnly: false, isViewMode: true, pairings: [{ player1Id: 1 }] })).toBe(false);
      expect(shouldShowReshuffleButton({ isReadOnly: true, isViewMode: false, pairings: [{ player1Id: 1 }] })).toBe(false);
    });
    it('ロック有無で文言が切り替わる（AC-2）', () => {
      expect(reshuffleButtonLabel([{ player1Id: 1, player2Id: 2, locked: true }])).toBe('ロックされた組以外をシャッフル');
      expect(reshuffleButtonLabel([{ player1Id: 1, player2Id: 2 }])).toBe('再シャッフル');
    });
  });

  describe('確認ダイアログ → 再シャッフルで送信 body に lockedPairs が入る（AC-3/4/6）', () => {
    // 本番 handleReshuffle と同じ経路（confirm → computeLockedPairsInput → buildAutoMatchBody → autoMatch）を
    // 実関数で再現する。confirm ゲートのみ薄く再現し、payload 生成・送信は本番と同一の実関数を通す。
    const reproduceReshuffle = async (pairings, confirmResult, sessionDate = '2026-06-28', matchNumber = 2) => {
      if (!confirmResult) return { sent: false };
      const body = buildAutoMatchBody(sessionDate, matchNumber, computeLockedPairsInput(pairings));
      await pairingAPI.autoMatch(body);
      return { sent: true };
    };

    it('確認 OK: ロック組（手動ロック＋未保存ロック）が lockedPairs として送信される', async () => {
      apiClient.post.mockClear();
      apiClient.post.mockResolvedValueOnce({ data: { pairings: [], waitingPlayers: [], lockedPairings: [] } });

      const pairings = [
        { id: 10, player1Id: 1, player2Id: 2, locked: true },   // 保存済み手動ロック
        { player1Id: 5, player2Id: 6, locked: true },           // 未保存ロック（id なし）
        { player1Id: 3, player2Id: 4, locked: false },          // 非ロック → 送らない
      ];
      const { sent } = await reproduceReshuffle(pairings, true);

      expect(sent).toBe(true);
      expect(apiClient.post).toHaveBeenCalledWith('/match-pairings/auto-match', {
    ...
    it('確認 OK・ロック0件: lockedPairs=[] を送る（全参加者を再シャッフル・AC-5）', async () => {
      apiClient.post.mockClear();
      apiClient.post.mockResolvedValueOnce({ data: { pairings: [], waitingPlayers: [], lockedPairings: [] } });

      const pairings = [
        { player1Id: 1, player2Id: 2, locked: false },
        { player1Id: 3, player2Id: 4, locked: false },
      ];
      await reproduceReshuffle(pairings, true);
    ...
    it('確認キャンセル: autoMatch を呼ばない（組み合わせ・待機者は変化しない・AC-6）', async () => {
      apiClient.post.mockClear();
      const pairings = [{ player1Id: 1, player2Id: 2, locked: true }];
      const { sent } = await reproduceReshuffle(pairings, false);
  ...
  describe('新規作成「対戦編集」は lockedPairs を送らない（後方互換・回帰）', () => {
    it('autoMatch body に lockedPairs キーが無い', async () => {
      apiClient.post.mockClear();
      apiClient.post.mockResolvedValueOnce({ data: { pairings: [], waitingPlayers: [], lockedPairings: [] } });

      // 本番 handleAutoMatch = () => runAutoMatch()（引数なし＝undefined）
      await pairingAPI.autoMatch(buildAutoMatchBody('2026-06-28', 1, undefined));
...
describe('作成UI（参加者一覧・常設「対戦編集」ボタン）の表示条件', () => {
  // 本番 PairingGenerator.jsx が import している判定関数（pairingDisplayLogic）を
  // テストでも import して直接検証する。条件式をテスト側にコピーせず実関数を呼ぶことで、
  // 本番側が過去の `!hasUnlockedPairings`（全組ロック時に作成UIが復活）へ退行した場合に
  // 確実に失敗させる。
  const editAction = (
    pairings,
    { participants = [{ id: 1 }] } = {},
  ) => resolvePairingEditAction({ pairings, participants });
  ...
  describe('参加者一覧セクション', () => {
    ...
    it('結果入力済みの組だけでも非表示', () => {
      expect(shouldShowParticipantSection({ pairings: [{ hasResult: true, locked: false }], isEditing: false })).toBe(false);
    });
    it('手動ロック済みの組だけでも非表示', () => {
      expect(shouldShowParticipantSection({ pairings: [{ hasResult: false, locked: true }], isEditing: false })).toBe(false);
    });
    it('未ロックの既存組でも非表示（結果未入力の試合と表示を一貫）', () => {
      expect(shouldShowParticipantSection({ pairings: [{ hasResult: false, locked: false }], isEditing: false })).toBe(false);
    });
  });

  describe('常設「対戦編集」ボタンの動作解決', () => {
    it('組み合わせが無く・参加者が居れば自動組み合わせ', () => {
      expect(editAction([])).toBe('auto-match');
    });
    // 既存組があるときに 'auto-match' へ退行すると、保存済みの組み合わせが
    // 黙って組み直されて失われる（このスキルの最重要な回帰ガード）。
    it('結果入力済みの組があれば編集モード切替（再生成しない）', () => {
      expect(editAction([{ hasResult: true, locked: false }])).toBe('edit-existing');
    });
    it('手動ロック済みの組があれば編集モード切替（再生成しない）', () => {
      expect(editAction([{ hasResult: false, locked: true }])).toBe('edit-existing');
    });
    it('未ロックの既存組があれば編集モード切替（再生成しない）', () => {
      expect(editAction([{ hasResult: false, locked: false }])).toBe('edit-existing');
    });
    it('参加者が0名なら空の編集モード（「選手追加」への導線を残す）', () => {
      expect(editAction([], { participants: [] })).toBe('empty-edit');
    });
    it('isReadOnly（他試合に未保存変更）ならボタン自体を無効化', () => {
