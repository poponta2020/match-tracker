...
const PlayerDetail = () => {
  ...
  const roleLabel = (role) => {
    ...
    return map[role] || { text: role, cls: 'bg-surface-disabled text-text-muted' };
  };
  ...
  if (error || !player) {
    return (
      <div className="space-y-4">
        <div className="bg-status-danger-surface border border-status-danger/20 rounded-lg p-4 text-status-danger text-sm">
          {error || '選手が見つかりませんでした'}
        </div>
        <button
          onClick={() => navigate('/players')}
          className="flex items-center gap-1 text-sm text-secondary hover:text-primary-hover"
        >
    ...
    <div className="max-w-7xl mx-auto">
      ...
      <div className="bg-surface border-b border-border-subtle shadow-sm fixed top-0 left-0 right-0 z-50 px-4 py-4">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          ...
          <h1 className="text-lg font-semibold text-text">選手詳細</h1>
          <button
            onClick={() => navigate(`/players/${id}/edit`)}
            className="flex items-center gap-1.5 px-3 py-1.5 text-sm text-secondary border border-secondary rounded-lg hover:bg-primary hover:text-text-inverse transition-colors"
          >
