...
1. ログイン画面（選手名 + パスワード）
   └── 2. トップページ（ロールに応じてメニューが変化）
       ...
       │
       ├── 10. 管理者メニュー（ADMINとSUPER_ADMINのみ表示）
       │   ├── 11. 練習日管理画面
       │   │   ├── 練習日一覧
       │   │   ├── 練習日新規作成
       │   │   ├── 練習日編集
       ...
       │
       ├── 13. スーパー管理者メニュー（SUPER_ADMINのみ表示）
       │   └── 14. 選手管理画面
       │       ├── 選手一覧
       │       ├── 選手新規登録
       │       ├── 選手編集（全選手）
...
**画面数**: 15画面（日付別試合一覧画面を廃止し、カレンダー画面に統合）
...
- `updated_by` も同じく設定
- 保存後、カレンダー画面（画面7）に遷移（入力した日付が選択された状態）
...
#### 動作
1. **日付選択時**
   - 練習日をクリック → 同画面内の下部に試合結果タブを表示
   - タブ構成: [第1試合目] [第2試合目] [第3試合目] ... （その日の試合数に応じて動的に生成）
...
2. **タブ表示**
   - 各タブには該当試合の結果を表示
     ...
     - 枚数差
     - コメント（一部表示、長い場合は省略）
   - 試合結果が未入力の場合は「未入力」と表示
   - 各試合結果をクリック → 試合詳細画面（画面8）に遷移

3. **結果入力ボタン**
   - 「この日の結果を入力」ボタンを画面に配置
   - クリック → 対戦結果入力画面（画面6）に遷移（日付が選択済み状態）
...
- **削除ボタン**
  - 表示条件: 自分が `player1_id` または `player2_id` の場合のみ
  - クリック → 確認ダイアログ → 削除実行 → カレンダー画面（画面7）に戻る
...
### 画面数: 15画面
- 全ユーザー共通: 15画面（日付別試合一覧画面を廃止し、カレンダー画面に統合）
