...
### 7. 管理者機能
- **選手管理**
  - 選手の新規登録（名前、パスワード、性別、利き手、級、段位、所属かるた会）
  - 選手情報編集
  - 選手削除（対戦記録がある場合は削除不可）
  - 管理者権限の付与/削除
...
- 対戦記録がある選手は削除不可（ON DELETE RESTRICT）
...
- 必要なインデックス:
  - `matches`: `idx_matches_date_players` (match_date, player1_id, player2_id)
  - `matches`: `idx_matches_date_match_number` (match_date, match_number)
  - `match_pairings`: `idx_session_match` (session_date, match_number)
  - `player_profiles`: `idx_player_date` (player_id, valid_from, valid_to)
