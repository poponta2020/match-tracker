...
| # | パス | ページコンポーネント | 主要子コンポーネント | 権限 | 説明 |
|---|------|---------------------|---------------------|------|------|
| 37 | `/admin/densuke` | `DensukeManagement.jsx` | 月ナビゲーション、URL入力、同期結果表示、未登録者チェックリスト、書き込み状況セクション | ADMIN+ | 伝助URL管理、手動同期実行、未登録者の確認・一括登録。書き込み状況セクションでアプリ→伝助への書き込み状態（最終実行日時・最終成功日時・エラー・書き込み待ち件数）を表示 |
