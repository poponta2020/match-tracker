...
| 18 | `/practice/cancel` | `PracticeCancelPage.jsx` | キャンセル専用カレンダー、試合選択チェックボックス（「当選」/「申込」バッジ付き）、キャンセル理由ラジオボタン、当日12:00以降確認ダイアログ | ALL | 参加キャンセル（WON / PENDING 登録日をハイライトしたカレンダー→試合選択→理由選択→確認ダイアログ）。試合一覧では WON は「当選」、PENDING は「申込」のラベルで区別表示。当日12:00（JST）以降のキャンセル時は「直前のキャンセルとなりますがよろしいですか？」の追加確認ダイアログを表示。PENDING（抽選フェーズ1の自分の申込）のキャンセルは補充募集・繰り上げ通知を発火せず、`status=CANCELLED, dirty=true` を設定して伝助同期で × として書き戻すのみ。クエリパラメータ `?year=YYYY&month=M` で初期表示月を指定可能（不正値時は現在月にフォールバック） |
