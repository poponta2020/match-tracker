...
| # | パス | ページコンポーネント | 主要子コンポーネント | 権限 | 説明 |
|---|------|---------------------|---------------------|------|------|
| 41 | `/settings/card-division` | `CardDivision.jsx` | 所属団体ごとの札分けブロック（readonly textarea + コピーボタン、LINE購読トグル、LINE未連携時の通知設定への導線） | ALL | 当日の札分け（各試合の出札ルール）をテキストで確認・コピーする画面。所属団体ごとに1ブロックを表示（複数所属時は団体名見出しあり）。当日セッションが無い団体は「本日は練習がありません」の空表示。テキストの閲覧・コピーは購読トグルの状態に依存せず常時可能。LINE購読トグル（既定OFF）は団体ごとに個別に操作でき、LINE未連携の場合はトグル付近に通知設定画面への導線を表示 |
...
- 設定グリッドから入る画面 → `/settings`（プロフィール、参加練習会、通知設定、札分け確認、メンター管理、カレンダー購読、組み合わせ作成、会場管理、抽選管理、LINEチャネル管理、LINE通知スケジュール）
