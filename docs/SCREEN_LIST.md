...
補足: `venue-reservation-proxy` はバックエンド Controller / Service 層とフロントエンド API クライアント / venue 判別ユーティリティまで実装中。現時点では新規画面・新規フロントエンドルートは追加されておらず、`/practice` の隣室予約導線は後続タスクで同プロキシ API に接続する。
