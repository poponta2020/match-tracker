...
| 9 | `/matches/:id` | `MatchDetail.jsx` | `MatchCommentThread`（メンター⇔メンティー間コメントスレッド。メンティー本人またはACTIVEメンターのみ表示。未通知コメントがある場合は「LINE通知を送信（N件）」ボタンを表示） | ALL | 試合詳細表示（自分のお手付き回数・個人メモを表示。メンター関係がある場合はコメントスレッドを表示） |
