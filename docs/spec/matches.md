...
- **盤面**: 敵陣（画面上・**180°回転**：上段が手前/下段が奥、敵陣右＝画面左）／自陣（画面下・自分視点）を畳表現で描画。各マス（敵自 × 左右 × 上中下段＝12エリア）を**「取った(左・緑)｜取られた(右・赤)」の2分割**にし、決まり字の**縦書きチップ**を配置。象限内の2分割は札数に応じて横幅を動的吸収（少ない側の未使用幅を多い側が使い、最小幅は保つ）。不明プールは陣の間に置き、残数の母数＝`50 − 枚数差`（指導試合・枚数差不明は50）。不明プールの札は**決まり字順**（1文字目＝むすめふさほせ…、同一1文字目内は五十音順。表示順のみで計数は不変）で並ぶ。
- **配置操作**: **タップ**（不明札を選択→マスをタップ。マス内に札があってもその上でよい。非選択時は配置済みチップのタップで不明に戻す）に加え、**ドラッグ&ドロップ**（不明→マス配置／マス→別マス移動／マス→不明解除）で行える（`@dnd-kit` の Pointer/Touch センサー）。盤面の折りたたみブロックに `data-swipe-ignore` を付与し、試合番号スワイプ切替と分離する。
...
- **保存後の遷移・閲覧**: 個人結果入力（**新規**）の保存成功後は `/matches/:id`（試合詳細）へ遷移する（編集モードは `/matches`、抜け番活動は `/` のまま不変）。試合詳細（`MatchDetail`）では**本人閲覧時のみ**、取り札を**読み取り専用盤面**（配置済みのみ・不明プールと操作を出さない）、お手付き詳細を**読み取り専用**で統合カード下に表示する（記録が無ければ非表示。メンター閲覧 `?playerId=` では取得も表示もしない。既存 `GET .../card-record` を流用しBE改修なし）。
