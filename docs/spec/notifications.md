...
- 認証情報（channel_secret, channel_access_token）はAES-256-GCM（アプリケーションレベル暗号化）でDB保存。JPA `AttributeConverter`（`converter/EncryptedStringConverter`）で `enc:v1:` 接頭辞付き暗号文として書き込み、読取時は平文で返す（getter 契約は不変）。鍵は環境変数 `LINE_ENCRYPTION_KEY`（base64 32バイト）。接頭辞なしのレガシー平文はパススルー（鍵不要）で共存し、鍵未設定でもLINE未使用の運用は起動可能（遅延検証）
