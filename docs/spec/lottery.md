...
定員超過の試合では、当落を2層のルールで決める（`docs/features/lottery-fair-share-rotation/requirements.md` を出典とする）。

- **ルール1（その日の一巡保証・確定）**: そのセッション（1日）でまだ取れた試合数（`todayTaken`）が最も少ない候補者から座席を埋める。`todayTaken` が大きい候補者は、それより少ない候補者が尽きるまで選ばれない。これにより、希望者が定員を超える日でも「1日1試合も取れない人」が構造的に発生しない
- **ルール2（直近30日の重み付き抽選・確率）**: `todayTaken` が同数で並ぶ候補内でのみ、直近30日の取得数（`recentTaken`＝そのセッション日付から遡って30日間に同一団体で取れた試合数）が少ない選手ほど当選しやすい重み付き抽選を行う。重みは `1 / (min(recentTaken, cap) + 1)`
- **キャップ（相対パーセンタイル）**: 重みの上限 `cap` は、固定値ではなく**その試合の候補者の `recentTaken` 分布から毎回算出**する。候補者の `recentTaken` を昇順に並べ、システム設定 `lottery_weight_cap_percentile`（団体別、デフォルト30）が示す nearest-rank パーセンタイル値を `cap` とする（候補者が少なく `cap` が最小値と一致する場合は `cap` を+1して最小値の候補を保護する）。`recentTaken` が `cap` 以上の候補は全員同一の重みとなり（既定=上位おおむね70%がフラット化）、`cap` を下回る候補だけが少なさに応じて有利になる。よく参加している人が毎回真っ先に落選候補になることを防ぎ、しばらく参加できていなかった人を保護する。基準が団体の固定値ではなくその試合の顔ぶれから決まるため、暦月の位置（月初/月末）に左右されない
- **集計期間**: `recentTaken` は暦月リセットではなく、そのセッション日付から遡る**直近30日のローリング**で算出する。抽選実行前から存在する WON（管理者手動追加・当日join・繰り上げ承諾・確定済みセッション分）を初期値として算入する。`OFFERED`（応答待ち）は定員には算入するが `recentTaken` には数えない
- **管理者指定優先選手**: 管理者が指定した選手は最上位で座席を確保する（維持）。優先選手だけで定員超過の場合は、優先選手同士に同じルール1・ルール2を適用する。希望を出しているセッションのみ適用。優先選手が落選した場合、キャンセル待ちの最上位に入る
- **キャンセル待ち番号**: 落選者に対して当選者選抜と**同じ選抜手続きを続行**し、引かれた順に番号を付与する（バケット順は当選時と同じく管理者優先プール→その他プールで、優先落選者がキャンセル待ちの最上位に入る）。**前試合からのキャンセル待ち番号の引き継ぎは廃止**し、毎試合この選抜手続きのみで順番を決める
...
| 重みキャップのパーセンタイル | `lottery_weight_cap_percentile` | 0〜100の整数 | 直近30日の重み付き抽選（ルール2）で使うキャップの算出基準。値を小さくするほど多くの人が横並びになり、よく参加している人も不利になりにくい。値を大きくするほど参加回数の少ない人がより強く優先される。未設定の団体はデフォルト30が使われる |
...
**抽選の仕組み説明セクション:**
画面内に、ルール1（その日の一巡保証）・ルール2（直近30日の重み付き抽選）・パーセンタイル設定の意味を、管理者・会員が読んで理解できる日本語で説明するセクションを表示する。記載内容は上記「抽選アルゴリズムの特徴」節の2ルールおよびキャップの説明と整合させる。旧「一般枠の最低保証割合」（`lottery_normal_reserve_percent`）カードは廃止され、抽選ロジックからも参照されない
...
4. 定員超過判定（capacity vs 参加希望者数）
   ↓
5. 定員内 → 全員WON、定員超過 → 2ルールで選抜
   - **管理者指定優先プール**: 希望を出している優先選手を最優先で座席に割り当て。優先選手だけで定員超過の場合は優先選手同士に下記2ルールを適用
   - **その他プール**: 残り定員に対し、①その日まだ取れていない人（`todayTaken`最小）から確定的に選抜し、②`todayTaken`が同数の候補内でのみ直近30日の取得数（`recentTaken`）による重み付き抽選（設定 `lottery_weight_cap_percentile` によるキャップ付き）を行う
   - 当選者: status = WON
   - 落選者: status = WAITLISTED + waitlist_number付与（優先落選者→その他落選者の順、いずれも選抜手続きで引かれた順に番号付与。前試合からの番号引き継ぎはしない）
   ↓
