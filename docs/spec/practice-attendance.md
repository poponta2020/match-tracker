...
- **当月扱い／来月扱いの区別**（`resolveAttendanceMode`）:
  ...
  - **来月扱い**（未来月・抽選確定なし）で**締切前**（`beforeDeadline`）: 全試合をトグル表示し登録済みは pre-check。外すと理由なし取消（`registerParticipations` の全置換で反映）。理由付きキャンセルセクションは非表示。
  - **来月扱いでも締切後**（`beforeDeadline=false`）: 既存参加はトグルに出さず理由付きキャンセルへ回し、参加セクションは未参加の追加登録のみ（当月扱いと同じ振り分け）。バックエンドの `registerAfterDeadline` は payload 省略分を削除しない（追加のみ）ため、既存を全置換 uncheck しても消えずサイレント no-op になるのを防ぐ（既存 `PracticeParticipation.isLockedRegistration` と同判定）。
