...
ADMIN+ ユーザーが任意のタイミングで Kaderu 同期 workflow を起動できる仕組み。定期 cron を廃止したため、これが予約→練習日 sync の唯一の起点となる（Kaderu 側で予約が成立したその場でボタンを押して練習日一覧に反映させる運用）。
...
予約→練習日 sync の唯一の起点（定期 cron は廃止）。ADMIN+ が練習日登録画面のボタンで同期を起動する経路。バックエンドが GitHub Actions の `workflow_dispatch` API を叩き、結果をイベントテーブルに記録、scheduler が完了検知して LINE 通知を返す。
