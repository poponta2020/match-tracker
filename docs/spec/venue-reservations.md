...
予約→練習日 sync は**手動起動のみ**（定期 cron は廃止）。主経路は練習日登録画面の「Kaderu取り込み」ボタン → `sync-kaderu-reservations-manual.yml`。

| 方式 | 仕組み | トリガー |
|------|--------|---------|
| 手動ボタン（主経路） | 練習日登録画面 → `sync-kaderu-reservations-manual.yml` を workflow_dispatch 起動 | 手動 |
| 直接手動起動 | GitHub Actions `workflow_dispatch`（`sync-kaderu-reservations.yml`）または `node sync-reservations.js --org <code>` | 手動 |

`sync-kaderu-reservations.yml` は hokudai → wasura の順に 2 ステップ実行する。2 ステップ目は `if: always()` を付与しており、hokudai が失敗しても wasura は独立して実行される。いずれかのステップが失敗すると workflow 全体は失敗扱いとなる。`concurrency` グループ `kaderu-reservation-sync` で重複起動を防止する。
...
ADMIN+ ユーザーが任意のタイミングで Kaderu 同期 workflow を起動できる仕組み。定期 cron を廃止したため、通常運用ではこのボタンが予約→練習日 sync の主たる起点となる（Kaderu 側で予約が成立したその場でボタンを押して練習日一覧に反映させる運用）。GitHub UI からの直接 `workflow_dispatch`（`sync-kaderu-reservations.yml`）は保守・フォールバック経路として残る。
...
- GitHub Actions 側の `concurrency: kaderu-reservation-sync` グループにより、手動ボタン経由の起動と直接 workflow_dispatch の重複も自動的に直列化される
...
| ファイル | 役割 |
|---|---|
| `.github/workflows/sync-kaderu-reservations-manual.yml` | 手動同期専用 workflow (workflow_dispatch + inputs.org)。hokudai 押下時は東区民センター同期（`sync-higashi-reservations.js`）も続けて実行する |
...
定期 cron は廃止し**手動起動のみ**。通常は hokudai の「Kaderu取り込み」ボタン押下時に `sync-kaderu-reservations-manual.yml` の hokudai ステップから続けて実行される（東区民は北大の会場のため hokudai 押下時のみ）。`sync-higashi-reservations.yml` の直接 workflow_dispatch もフォールバック経路として残る。

| 方式 | 仕組み | トリガー |
|------|--------|---------|
| 手動ボタン（主経路） | hokudai 押下時に `sync-kaderu-reservations-manual.yml` から `sync-higashi-reservations.js` を実行 | 手動 |
| 直接手動起動 | GitHub Actions `workflow_dispatch`（`sync-higashi-reservations.yml`）または `node sync-higashi-reservations.js` | 手動 |

同一 Render PostgreSQL を共有（secret `KADERU_DATABASE_URL` を流用）。東区民同期は `sync-kaderu-reservations-manual.yml` の hokudai ステップ内（`kaderu-reservation-sync` グループ）で実行される。直接 `sync-higashi-reservations.yml`（`higashi-reservation-sync` グループ）を起動した場合はグループが別なので直列化されず並走しうるが、`sync-higashi-reservations.js` は idempotent な upsert のため二重起動でも壊れない。1日に両センターの予約が併存しない前提（Q13）。既存セッションがかでる系会場の場合、東区民センター同期側からは触らない。
...
予約→練習日 sync の主経路（定期 cron は廃止。GitHub UI からの直接 `workflow_dispatch` は保守用に残る）。通常運用では ADMIN+ が練習日登録画面のボタンで同期を起動する。バックエンドが GitHub Actions の `workflow_dispatch` API を叩き、結果をイベントテーブルに記録、scheduler が完了検知して LINE 通知を返す。
...
[GitHub Actions: sync-kaderu-reservations-manual.yml]
  workflow_dispatch (inputs.org)
   - concurrency: kaderu-reservation-sync (直接 workflow_dispatch と直列化)
   - if: inputs.org == 'hokudai'/'wasura' で1団体のみ実行
   - node sync-reservations.js --org <code> --months 2
   - hokudai のみ: 続けて node sync-higashi-reservations.js --months 2（東区民センター）
   ↓
...
```
[GitHub Actions: sync-higashi-reservations.yml（直接）
              または sync-kaderu-reservations-manual.yml の hokudai ステップ]
  workflow_dispatch（定期 cron は廃止）
  ↓
