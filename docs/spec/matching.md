...
- アルゴリズム:
  ...
  3. **同一団体の「前回練習日」ペア集合を特定**（下記）
  4. 各候補ペアに対してスコアを計算（直近対戦からの日数が遠いほど高スコア。前回練習日ペアには固定ペナルティを加算）
  5. 貪欲法で最高スコアのペアから順に確定
  6. 奇数人数の場合は1名が待機者となる
- スコア計算: `-(100.0 / 最終対戦からの日数)`。初対戦は `0`、同日対戦は `-1000`
- **前回練習日ペアの強い回避（pairing-avoid-previous-practice-opponents）:** 同一団体の「前回練習日」に組んだペアは、上記スコアに固定ペナルティ（`PREVIOUS_PRACTICE_PENALTY = -1000`）を**加算**して強く回避する。
  - 「前回練習日」= 今回のセッション日より前で、実際に対戦（`match_pairings` / `matches`）がある直近の練習日1日分。直前の練習日が対戦なし（中止・記録なし）の場合は対戦がある日までさらに遡る（`findPastSessionDatesByOrganizationId` を降順に walk、最大 `MAX_PREV_LOOKBACK=20` 件）。過去30日窓には依存しない（30日より前でも回避対象）。
  - **同一団体スコープ**で特定する（`getSessionAllPlayerIds(day, org)` の参加者で両選手 AND フィルタ）。別団体の同日ペアはペナルティ対象に混入しない。SUPER_ADMIN（`organizationId=null`）実行時は当日セッションから解決した実効団体IDを使い、解決できなければ空集合（ペナルティなし）。
  - **ソフトペナルティ（グレースフル劣化）:** 固定ペナルティは有限値のため、他に組める相手がいる限り前回練習日ペアは再形成されないが、それ以外に組みようがない少人数ケースでは待機者を増やさず最後の手段として許容する。同日ペアの**完全除外**（手順2）はこの緩和の対象外で従来どおり維持する。
...
- 前回練習日ペア（同一団体）: 上記スコアに固定ペナルティ -1000 を加算して強く回避（ソフト＝グレースフル劣化。詳細は上記「前回練習日ペアの強い回避」）
- 貪欲法で最適ペアリング
...
- **テキスト形式**: 1行目ヘッダ `M/D(曜) 会場名`（すべて半角。囲みかっこ `【】` は付けない。曜日は日付から算出＝`月火水木金土日`。月・日は10の位0を省略）→**空白行1行**→各行 `N試合目：<札ルール>`。一の位・十の位の行は位のあとに全角スペースを挟む（`一の位　1.3.5.6.7`）。抜き行のみ `番号(決まり字)抜き`（例 `41(こひ)`、`100(もも)`）。決まり字マスタは `util/Kimariji.java`（`kimariji.js` の `KIMARIJI` を補正値込みで複製）。抜き札番号は `parseInt(removedCard)||100`（"00"→100）。対戦ペアは載せない（札組のみ）。
- **会場名・試合数・nonce**: `PracticeSession`（date+org）→ 会場名（`Venue`）・`totalMatches`・`CardRuleNonceService.getNonce(date)` から解決。会場未設定なら `M/D(曜)`。当日該当団体のセッションが無ければ `hasSession=false`・`text=null`。
...
9. 同日既存対戦を除外
   ↓
10. 同一団体の前回練習日ペア集合を特定（スコアに固定ペナルティを加算して強く回避）
   ↓
11. 参加者シャッフル → 貪欲法でスコア最高ペアを選択
   ↓
12. レスポンス: AutoMatchingResult（ペア一覧 + 待機者リスト + ロック済みペア一覧）
   ↓
[ユーザー操作]
13. 手動調整 → 「組み合わせ確定」ボタンクリック
   ↓
[バックエンド]
14. ロック済みペアリングを保持しつつ、未ロック分を削除 → 新規一括登録
```
