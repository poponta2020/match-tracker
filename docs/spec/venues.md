...
1. **スクレイピング（GitHub Actions）** — かでるのみ自動。
   ...
   - 東区民センター（東🌸）: **自動スクレイプは廃止**。`scrape-higashi-availability.yml` は `workflow_dispatch`（手動実行）のみ残す。会場拡張はスクレイプ非依存の手動化（下記）
2. **通知判定（バックエンド `AdjacentRoomNotificationScheduler`, 毎時0分＝JST 1〜5時台スキップ）** — かでる4室のみ。
   - 翌日〜40日先の全セッションから `AdjacentRoomConfig.isAdjacentNotificationTarget`（かでる4室のみ。東🌸は対象外）で対象をフィルタ
...
**東🌸（東区民センター）の会場拡張は手動化**（自動通知・空き検証なし）。`AdjacentRoomConfig` は3概念を分離する: **隣室チェック対象**（`isAdjacentCheckTarget`＝かでる4室＋東🌸。DTO・拡張先マッピングの供給元。変更しない）／**自動通知対象**（`isAdjacentNotificationTarget`＝かでる4室のみ）／**手動拡張会場**（`isManualExpansionVenue`＝チェック対象かつ非通知＝東🌸(6)のみ）。東🌸セッションでは `AdjacentRoomStatusDto.manualExpansion=true` が返り、FE は隣室 status チップと空き依存ゲートを外して「会場を拡張（東全室に）」を管理者に常時表示。`expandVenue` は手動拡張会場のみ `reservationConfirmedAt` 検証と隣室 `expandable` 検証をスキップし（確認は UI ダイアログで担保）、拡張先取得〜定員更新〜キャンセル待ち繰上げの共有ロジックはかでると共通。
...
>
> 空き通知（処理フロー2.）の条件は `available`、会場拡張（`expandVenue`）の可否は `expandable` を用いる。`●` は Kaderu 和室でもオンライン予約（プロキシ）をスキップし、「予約完了を報告 → 会場を拡張」の手動フローで扱う（UI フローは「フロー」節の「隣室予約→会場拡張フロー」参照）。**かでる**の `expandVenue` は `reservationConfirmedAt != null` かつ隣室が `expandable` であることをサーバー側で再検証する。**東🌸(6)（手動拡張会場）はこの2検証をスキップ**し、予約済み前提で拡張する（`isManualExpansionVenue`）。
...
| `.github/workflows/scrape-higashi-availability.yml` | 東区民センター側スクレイプ（**自動廃止**・`workflow_dispatch` 手動のみ） |
...
> ※ 上表は東区民センター機能の初回実装時点の内容。**2026-07-20 の軽量化（[adjacent-room-check-lightweight](../../features/adjacent-room-check-lightweight/requirements.md)・#1135）で東🌸の自動確認を廃止**: スケジューラーのフィルタは `isAdjacentNotificationTarget`（かでる4室のみ）に変更、`scrape-higashi-availability.yml` の `schedule:` は削除（`workflow_dispatch` のみ）、東🌸の会場拡張はスクレイプ非依存の手動化。現行挙動は本節冒頭の「処理フロー」を正とする。
