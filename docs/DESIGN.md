...
- **取得**: 抜け番は `type` 指定なしで全件取得し、フロントで READING / SOLO_PICK のみに絞る。取得は `targetPlayerId` 変更時のみ（フィルタ変更では再取得しない）。失敗時は抜け番非表示でフォールバック。
...
- **期間フィルタ候補**: 年/月セレクトの候補（`availableYears` / `availableMonths`）は試合の `matchDate` と抜け番の `sessionDate` の**両方**から生成する（試合がなく抜け番のみの年月にも期間フィルタで到達できるようにするため）。
