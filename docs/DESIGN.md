...
#### GET /ical/calendar/{token}.ics
**説明**: 自分の参加予定練習をiCal形式で配信。Googleカレンダー等の購読クライアントから直接取得される。
**権限**: 認証なし（推測困難なtokenが事実上の認証。論理削除済みプレイヤーは404）
**レスポンス**: `Content-Type: text/calendar;charset=UTF-8` 形式のテキスト

#### GET /api/calendar/feed/info
**説明**: 自分のフィードURLと所属団体・表示名一覧を取得
**権限**: ALL（自分自身のリソースのみ）
**レスポンス**: `FeedInfoDto` (`{ url, organizations: [{ organizationId, organizationName, displayName }] }`)

#### POST /api/calendar/feed/regenerate
**説明**: iCalフィードトークンを再発行（旧トークンは即時無効）
**権限**: ALL
...
- **今日の日付をハイライト**
- **出欠登録ボタン（右下フローティング、常時表示）**: 押下で `AttendanceRegisterModal` を開く。モーダル内で「参加登録」「キャンセル登録」を選択し、それぞれ `?year=YYYY&month=M` 付きで該当ページへ遷移。旧仕様の「参加登録」「参加キャンセル」2フローティング構成は本モーダル統合で「出欠登録」1ボタンに集約された
- **日付クリック→モーダルポップアップ**
...
#### カレンダー購読（iCalフィード）
- プレイヤーごとに発行された `ical_feed_token` を含む公開URL `/ical/calendar/{token}.ics` を購読側（Googleカレンダー等）が定期取得（数時間〜半日間隔）
- biweekly でリクエスト時にDB状態を反映したVCALENDARを動的生成（サーバ側スケジューラ不要）
- 削除・キャンセルされた練習は次回フェッチで自動消去（VEVENT.UID で同一性判定）
- VEVENT.UID: `session-{sessionId}-player-{playerId}@match-tracker`
- 表示名カスタマイズ: `player_organizations.calendar_display_name` で団体ごとに上書き可
- ゲスト参加（未所属団体の練習）も含める、表示名は `Organization.name` 固定
- `CANCELLED` ステータスの参加は除外
