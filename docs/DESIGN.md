...
#### 練習セッション更新時の一意制約違反対策（2025-11-20追加）
...
- 効果: 削除操作が即座にDBに反映され、一意制約違反を回避
- 該当制約: `uk_session_player_match` (session_id, player_id, match_number の一意制約)
...
#### v1.2.0（2025-11-20）
- **今日の対戦カード機能（ホーム画面）**
  - 今日の練習セッションと自分の対戦情報を表示
  - 対戦相手名のみ表示（自分の名前は非表示）
  - 試合結果入力済みの場合、「対戦相手名〇11」形式で結果表示
  - 未入力の場合、「タップして結果を入力」ボタン表示
  - 試合切り替えタブで複数試合に対応
  - 未入力の最小試合番号を優先表示
  - 全試合入力済みの場合は最後の試合を表示
  - 画面フォーカス時に自動再取得（試合結果入力後の更新対応）
- **試合結果入力フォーム改善**
  - ホーム画面からの遷移時に日付・試合番号・対戦相手を自動入力
  - location.state経由でデータを引き継ぎ
- **バグ修正**
  - 練習セッション更新時の参加者重複エラーを解消
  - `practiceParticipantRepository.flush()`追加で削除操作を即座に反映
  - 一意制約違反（uk_session_player_match）の問題を修正
