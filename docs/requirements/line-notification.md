...
|----|------|------|
| 1.0 | 2026-03-24 | 初版作成 |
| 1.1 | 2026-03-25 | ワンタイムコードによるアカウント紐付け方式を追加 |
...
| ワンタイムコード | LINE友だち追加後にアプリのプレイヤーとLINEアカウントを紐付けるための一時的な認証コード。英数字8桁、有効期限10分 |
...
[事前準備(手動)]         [運用(自動)]                    [回収(自動)]
 ...
 を一括作成                  │                          警告通知(アプリ内)
      │                 未使用チャネルを割り当て               │
 認証情報をDBに         ワンタイムコード発行                猶予期間経過
 登録                        │                              │
      │                 友だち追加URL表示                割り当て解除
 Webhook URL設定        ユーザーが友だち追加              チャネル→AVAILABLE
                             │
                        Webhook: follow受信
                        Botが「連携コードを入力してください」と返信
                             │
                        ユーザーがLINE上でコードを送信
                             │
                        Webhook: messageイベント受信
                        コード検証 → line_user_id保存
                        → LINKED状態
```
...
2. 各チャネルのWebhook URLを設定: `https://{domain}/api/line/webhook/{lineChannelId}`（例: `/api/line/webhook/1234567890`）
   - `{lineChannelId}` はLINE Developers Consoleで発行されるチャネルID（DB内部IDではない）
...
システム: AVAILABLEなチャネルを1つ割り当て（→ ASSIGNED）
    │
システム: ワンタイムコード（英数字8桁、有効期限10分）を発行
    │
システム: 友だち追加URLとワンタイムコードを画面に表示
         ※ コードはコピペ前提のためコピーボタンを設置
    │
ユーザー: 友だち追加URLからLINEアプリで友だち追加
    │
LINE → Webhook: followイベント送信
    │
システム: follow受信。Botから「連携コードを入力してください」とリプライ送信
    │
ユーザー: LINEトーク画面でワンタイムコードを貼り付けて送信
    │
LINE → Webhook: messageイベント送信（テキスト = ワンタイムコード）
    │
システム: コードを検証
    ├─ 一致 → line_user_idを保存（→ LINKED）、「連携が完了しました」と返信
    └─ 不一致/期限切れ → 「コードが無効です。アプリから再発行してください」と返信
    │
...
| 項目 | 値 |
|------|-----|
| 形式 | 英数字（a-z, A-Z, 0-9）8桁 |
| 有効期限 | 発行から10分 |
| 再発行 | 有効期限切れ後、アプリから再発行可能。再発行時は旧コードを無効化 |
| 試行回数制限 | 5回失敗で当該コードを無効化（ブルートフォース対策） |
| コピー支援 | アプリ画面にコピーボタンを設置し、コピペを前提としたUX |
...
| code | VARCHAR(8) | NO | - | 英数字8桁のワンタイムコード（UNIQUE） |
| status | VARCHAR(20) | NO | 'ACTIVE' | ACTIVE / USED / EXPIRED / INVALIDATED |
| attempt_count | INTEGER | NO | 0 | 検証失敗回数（5回で無効化） |
| expires_at | TIMESTAMP | NO | - | 有効期限（発行から10分後） |
| used_at | TIMESTAMP | YES | - | 使用日時 |
| created_at | TIMESTAMP | NO | NOW() | 発行日時 |
...
LINEプラットフォームからのWebhookを受信する。`{lineChannelId}` はLINE Developers Consoleで発行されるチャネルID。

- **認証**: x-line-signature ヘッダーによる署名検証（チャネルごとのchannel_secretを使用）
- **処理するイベント**:
  - `follow`: Botから「連携コードを入力してください」とリプライ送信。この時点ではまだLINKEDにしない
  - `message`（テキスト）: ワンタイムコードとして検証。一致すればline_user_idを保存し割り当てステータスをLINKEDに更新、不一致/期限切れならエラーメッセージを返信
...
- **処理**: AVAILABLEなチャネルを割り当て、ワンタイムコードを発行し、友だち追加URLとコードを返す
...
ワンタイムコードを再発行する（期限切れ時）。

- **リクエスト**: `{ "playerId": Long }`
- **レスポンス**: `{ "linkingCode": String, "codeExpiresAt": String }`
- **処理**: 既存コードを無効化し、新しいコードを発行。チャネルが ASSIGNED 状態の場合のみ実行可能
- **エラー**: チャネル未割り当て時は400
...
全チャネルのWebhook URLをLINEチャネルIDベースに一括移行する。LINE Messaging APIを呼び出し、各チャネルのWebhook URLを `/api/line/webhook/{lineChannelId}` 形式に更新する。
...
| 画面名 | パス | 対象ロール | 概要 |
|-------|-----|----------|------|
| LINE通知設定 | /settings/line | 全ロール | LINE連携の有効化/無効化、友だち追加URL・ワンタイムコード表示（コピーボタン付き）、通知種別ON/OFF |
...
| Webhook URL | チャネルごとにLINE Developers Consoleで設定が必要。URLは `/api/line/webhook/{lineChannelId}` 形式（LINEチャネルIDベース）。一括移行エンドポイント `POST /api/admin/line/channels/migrate-webhook-urls` で既存チャネルの移行が可能 |
