...
|----|------|------|
| 1.0 | 2026-03-24 | 初版作成 |
| 1.1 | 2026-03-24 | キャンセル待ち連絡のPostback対応（LINE上での参加/不参加選択）を追加 |
...
| ボタンテンプレート | テキストと選択肢ボタンを含むLINEの構造化メッセージ。ユーザーがボタンを押すとpostbackイベントが発生する |
| Postback | ユーザーがボタンテンプレートのボタンを押した際にWebhookに送信されるイベント。任意のデータ文字列を含む |
...
| 2 | キャンセル待ち連絡 | イベント発火型（自動） | キャンセル発生→繰り上げ対象者へのオファー時に自動送信。ボタンテンプレートで参加/不参加を選択可能 |
...
- キャンセル待ち連絡: `WaitlistPromotionService.promoteNextWaitlisted()` 実行時。ボタンテンプレートで送信し、LINE上で直接参加/不参加を回答可能（詳細は第3.4節）
...
| 2 | キャンセル待ち連絡 | ボタンテンプレート形式（詳細は第3.4節） |
...
### 3.4 キャンセル待ち連絡のPostback対応

キャンセル待ち連絡はテキストメッセージではなく、**ボタンテンプレートメッセージ**で送信し、ユーザーがLINE上で直接参加/不参加を回答できるようにする。

#### 3.4.1 送信メッセージ形式

ボタンテンプレート（Button Template Message）を使用する。
...
{
  "type": "template",
  "altText": "繰り上げ参加のご連絡: 4月5日の練習 試合1",
  "template": {
    "type": "confirm",
    "text": "4月5日の練習 試合1に空きが出ました。参加しますか？\n（回答期限: 4/4 10:00）",
    "actions": [
      {
        "type": "postback",
        "label": "参加する",
        "data": "action=accept&participantId=123"
      },
      {
        "type": "postback",
        "label": "参加しない",
        "data": "action=decline&participantId=123"
      }
    ]
  }
}
...
- `altText`: ボタンテンプレートを表示できない環境向けの代替テキスト
- `confirm`テンプレート: テキスト + 2つのボタンを表示するLINE標準テンプレート
- `postback` action: ボタン押下時にWebhookへpostbackイベントを送信する。ユーザーのトーク画面にはテキストは表示されない

#### 3.4.2 Postback受信時の処理フロー
...
LINE → Webhook: postbackイベント
    │  data = "action=accept&participantId=123"
    │
システム: participantIdからPracticeParticipantを取得
    │
    ├─ ステータスが OFFERED の場合（未回答）
    │   │
    │   ├─ action=accept → WaitlistPromotionService.respondToOffer(id, true)
    │   │   → LINE返信: 「参加登録が完了しました！」
    │   │
    │   └─ action=decline → WaitlistPromotionService.respondToOffer(id, false)
    │       → LINE返信: 「不参加で登録しました」
    │
    └─ ステータスが OFFERED 以外の場合（回答済み or 期限切れ）
        │
        └─ LINE返信: 「この繰り上げ参加は既に処理済みです」
```

#### 3.4.3 二重回答の防止

LINEのメッセージは送信後に編集・削除できないため、ボタンは回答後もユーザーのトーク画面に残り続ける。以下の仕組みで二重処理を防止する。

| シナリオ | 挙動 |
|---------|------|
| LINEのボタンで回答（初回） | 正常処理。アプリ側の参加/不参加を登録し、確認メッセージを返信 |
| LINEで回答済み → 再度ボタン押下 | ステータスがOFFERED以外のため「既に処理済みです」を返信 |
| アプリで先に回答 → LINEのボタン押下 | ステータスがOFFERED以外のため「既に処理済みです」を返信 |
| LINEで先に回答 → アプリで操作 | アプリ側も既存のステータスチェックで弾く（既存実装で対応済み） |
| オファー期限切れ後にボタン押下 | ステータスがDECLINEDに変更済みのため「既に処理済みです」を返信 |

#### 3.4.4 確認メッセージ

Postback処理完了後、結果をテキストメッセージでユーザーに返信する。これにより：
- ユーザーが自分の回答結果を確認できる
- トーク画面の最新メッセージが確認メッセージになるため、古いボタンが目立たなくなる

| 処理結果 | 確認メッセージ |
|---------|-------------|
| 参加登録完了 | 「参加登録が完了しました！4月5日の練習 試合1でお待ちしています」 |
| 不参加登録完了 | 「不参加で登録しました」 |
| 処理済み（二重回答） | 「この繰り上げ参加は既に処理済みです。最新の状況はアプリでご確認ください」 |

※ 確認メッセージの送信は月200通の上限にカウントされる。キャンセル待ち連絡1件あたり最大2通（ボタンテンプレート + 確認メッセージ）を消費する。
...
- **処理するイベント**:
  ...
  - `postback`: キャンセル待ち連絡の参加/不参加回答を処理（詳細は第3.4節）
...
| 統合箇所 | 既存処理 | 追加内容 |
|---------|---------|---------|
| `WaitlistPromotionService.promoteNextWaitlisted()` | アプリ内通知 + Web Push | LINE送信（キャンセル待ち連絡・ボタンテンプレート）を追加 |
| `WaitlistPromotionService.respondToOffer()` | ステータス更新 | LINE Postbackからの呼び出しに対応（既存ロジックをそのまま利用） |
...
※ Postbackによるオファー回答は、既存の`WaitlistPromotionService.respondToOffer()`をそのまま呼び出す。回答チャネル（LINE / アプリ）による処理の分岐は不要。
