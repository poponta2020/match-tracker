...
- **試合番号は数字タブ＋滑る cream ハイライト**（`pairings-match-tab-bar` で下線タブから改修）：全タブ同一フットプリントを左寄せ表示、アクティブは紺文字＋滑る cream ハイライト（`#ebe4d8`）で連結パネルに接続（≤7 は1行・8以上は横スクロール）。「N試合目」はパネル冒頭の静的見出しで保持。完了は緑チェック、未保存はオレンジ文字。
...
| AC-3 | 試合番号は数字タブ表示（`pairings-match-tab-bar` で滑る cream ハイライトに改修）。アクティブは紺文字、作成済みは緑チェック、未保存タブは琥珀文字で区別される | verify |
