...
**優先順位:**
1. **createdBy が同日の PracticeParticipant として参加している practice_session の venue_id**
   - `PracticeParticipantRepository.findBySessionDateAndPlayerId(match_date, createdBy)` 相当のクエリ
   - 該当 session の `venue_id` が NULL でなければそれを使う
...
| 6 | venue_id 決定ロジック | PracticeParticipant → 同日 practice_sessions（一意なら採用） | createdBy の所属団体経由で任意1件採用 | 任意採用は将来複数会場時に誤った venue を入れるリスク。`参加実績` を最優先しつつ、フォールバックは同日一意の場合のみ採用するセーフティ付き |
