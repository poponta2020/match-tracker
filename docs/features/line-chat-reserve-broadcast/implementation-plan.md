...
### タスク8: Phase 3 VMデプロイ＋本番投入（運用・ユーザー協働）
- [~] **8a（VMデプロイ＋smoke）完了（2026-07-18）** / 8b（本番投入）未実施
- **目的:** Oracle VMへデプロイし、VM上でスタックとセッションが通ることを実測してから本番グループへ投入する
- **対応AC:** AC-12
- **主な変更領域:** VM上の docker compose 環境（コードはタスク6の資材）、`line_broadcast_group` への本番グループ識別情報登録、OAの本番全体グループへの招待
- **【方針変更 2026-07-18】2週間の事前観測は廃止**（理由は requirements.md §7・変更履歴）。「VM smoke＋本番初回の目視確認」に差し替え。
- **8a: 完了済み（実測ログあり）**
  - Oracle VM（kagetra_new 稼働ホスト。**SSH接続情報は `CLAUDE.local.md`**）の `~/line-chat-worker` へ配置・`docker compose` 常駐起動（`restart: unless-stopped`）。kagetra_new（compose名 `docker`）とは network/volume/project 名で完全分離
  - Render に `LINE_CHAT_WORKER_TOKEN` を設定（**未設定だと全ポーリングが401＝「タスク無し」と見分けがつかない**）。**Render は env 変更だけでは再デプロイされない**ため明示デプロイが必要
  - **Smoke A（トークン疎通）PASS**: VMから トークン無し=401 / 不正=403 / 正当=200+`[]` を実測
  - **Smoke B（VMのIPでのセッション疎通）PASS**: 実ワーカーコード（`OamChatPage`+`reserveMessage`）でテストグループへ dry-run → `DRY_RUN_SUCCEEDED`。スクショで「送信日時を設定」モーダルに日時投入済み・本文の改行3行保持・「設定」未押下を確認
  - 常駐後、`storage-state.json` のローリング再書き出しを確認＝サイクル完走（失敗時は `cycle failed` がログに出る設計）
- **8b: 残（ユーザー判断が必要）**
  - ①**本番OAの確定**: 現在 `.env` の `LINE_OAM_ACCOUNT_PATH` はT7のテストOA（ドラゴン）。本番全体グループに参加させるOAを決め、そのアカウントパスに差し替える。**storageStateはBusiness ID単位で同一セッションが全OAに通る**ことを実測済みのため、OA変更は env 差し替えのみでよい（再ログイン不要）
  - ②OAを本番全体グループへ招待し、そのルームの `chatRoomId`(`C…`) を取得
  - ③`line_broadcast_group` id=1（北海道大学かるた会 全体グループ）に `chat_room_id`/`chat_room_name` を登録し `enabled=true` にする（**現在は `enabled=f`・chat_room_id=null のため予約は1件も生成されない＝安全な停止状態**）
  - ④`docker compose restart` → 最初の1〜2回の予約を送信前にOAM上で目視確認 → 問題なければ完全自動
- **依存タスク:** タスク7
- **完了条件:** 本番グループでの初回自動配信成功
- **対応Issue:** #1093
