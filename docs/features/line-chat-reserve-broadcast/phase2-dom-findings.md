...
## 12. 【最重要】予約の有無は DOM バナーではなく予約一覧APIで判定する（2026-07-18 発見）

**事象**: 本番グループで、予約バナーが実在するのに `findDuplicateReservation` が `false` を返し、
続く `deleteReservation` も削除に失敗した（サイレント失敗）。

**原因**: バナーDOMの描画は `#editor` の出現と独立しており、`openChat` の固定待ち（1.5秒）では
間に合わないことがある。**「バナーが見つからない」を「予約が無い」と解釈していた**のが誤り。
待ち時間を延ばしても解決しない — *不在*の確定は固定タイムアウトでは原理的に行えず、
タイムアウトがそのまま「予約なし」の誤判定＝**二重予約（人数分の重複配信）**になる。

- セレクタ自体は正しかった（実測で確認）。バナーは `div[role=alert].alert` の**子孫**にテキストを持つため、
  `getByRole("alert").filter({hasText})` は祖先マッチで正しく引っかかる。純粋なタイミング競合。

**対策**: ルーム表示時に OAM 自身が叩いている**予約一覧API**を唯一の根拠にする（DOM 非依存）。

```
GET /api/v1/bots/<accountPath>/chats/<chatRoomId>/messages/scheduled
→ {"list":[{"scheduledMessageId":"agpx…","message":{"text":"…","type":"textV2"},
            "scheduledAt":1788217200000,"status":"SCHEDULED"}]}
```

- `scheduledAt` は **epoch ミリ秒**。JST壁時計・分精度に変換して `scheduledSendAt` と比較する。
- `status` が `SCHEDULED` のものだけを対象にする。
- **1ルームに複数予約が並存しうる**（取消→再予約で送信時刻が変わった場合など）ため、
  「list が空でない＝重複」ではなく必ず日時一致で判定する。
- **本文（`message.text`）では絞らない**。同一日時で本文だけ異なる予約を「別物＝重複なし」と
  判定すると二重予約になるため、日時一致のみで保守的に重複とみなす。

**フェイルセーフ規則（最重要）**: 非200・JSON不正・想定外の形・通信失敗は**すべて UNKNOWN** とし、
`NONE`（予約なし）に潰さない。予約を作ってよいのは「200応答を解釈でき、対象日時の予約が無いと確定できた」ときだけ。
DOM 判定はマークアップ変更時に黙って「0件」を返して二重予約に至るが、API 判定は形が変われば UNKNOWN に落ちて
人手確認に回る（危険側ではなく安全側に壊れる）。

- 削除も同様に、**削除できたことを一覧APIで確認できるまで `CANCELLED` を報告しない**
  （残ったまま「取消済み」と記録されると誤配信に気づけない）。
- DOM が必要なのは「⋮→削除」のクリック操作のみ。存在確定後に待つので、
  バナーが出なければそれは異常＝`UNKNOWN`（`NOT_FOUND` ではない）。
