...
**データ取得:**
- 既存の `GET /api/bye-activities/player/{playerId}`（`type` 指定なし）で全抜け番を取得し、フロントで `READING` / `SOLO_PICK` のみに絞る（`targetPlayerId` 変更時のみ取得。フィルタ変更では再取得しない）
...
**フィルタとの関係:**
- 期間（年/月）フィルタ: 読み・一人取り行・回数表示の両方に常に連動
