...
隣室予約時に、管理者のブラウザをアプリの `/api/venue-reservation-proxy/*` 経由で会場サイトへ中継し、予約申込トレイ表示から申込完了検知までを自ドメイン内で扱う。現時点ではバックエンド Controller / Service 層、フロントエンド API クライアント / venue 判別ユーティリティ、`PracticeList.jsx` の隣室予約導線接続まで実装済み。
