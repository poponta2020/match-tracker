...
- **ボタン:**
  - 「参加登録」: `/practice/participation?year=YYYY&month=M` へ遷移してモーダルを閉じる
  - 「キャンセル登録」: `/practice/cancel?year=YYYY&month=M` へ遷移してモーダルを閉じる
  - 「閉じる」: 遷移せずモーダルを閉じる
- **配置:** カレンダー画面の右下フローティング「出欠登録」ボタン（常時表示）と、選択セッション詳細部のインライン「出欠登録」ボタン（過去日でない場合のみ表示）の双方から開く
...
**フロー:**
1. キャンセル専用カレンダーが表示され、自分にキャンセル可能な参加（WON または PENDING）がある日がハイライト
   - **当選を含む日**: 赤系の色でハイライト（試合数を赤字で表示）
   - **申込のみの日（抽選前）**: 青系の色でハイライト（試合数を青字で表示）
2. 日付を選択すると、その日の自分の参加試合一覧が表示される。各試合には「当選」または「申込（抽選前）」のバッジが付与される
...
| `/practice/participation` | 出欠登録 | ALL | 月別カレンダーで参加登録。試合番号チェック欄はチェックボックス周囲もタップ可能。`?year=YYYY&month=M` で初期表示月を指定可（カレンダー画面の出欠登録モーダル経由で年月引き継ぎ） |
| `/practice/cancel` | 参加キャンセル | ALL | 理由付きキャンセル専用ページ。`?year=YYYY&month=M` で初期表示月を指定可（カレンダー画面の出欠登録モーダル経由で年月引き継ぎ） |
