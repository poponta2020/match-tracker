...
**ステータス別の挙動:**
- WON のキャンセル: 従来どおり、当日12:00以降は当日補充フロー、それ以前はキャンセル待ち繰り上げフローが発火する。
- PENDING のキャンセル: 抽選未実行（フェーズ1）のため、`status=CANCELLED, dirty=true` のみ設定。繰り上げ通知・当日補充通知は発火しない。`dirty=true` により伝助同期で当該プレイヤーは × として書き戻される。
