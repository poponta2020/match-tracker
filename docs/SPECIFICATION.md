...
- **LINE Messaging API SDK v9.14.0** — LINE通知送信
...
#### 概要
LINE Messaging API（Push API）を利用し、ユーザーに各種通知をLINEで送信する。
無料プランの月200通制限を回避するため、ユーザー1人に対しLINEチャネル（Messaging APIアカウント）を1つ割り当てる方式を採用。

#### 通知種別

| 種別 | トリガー | 説明 |
|------|---------|------|
| 抽選結果 | 管理者手動 | 抽選結果確認画面から管理者が「LINE通知送信」を押下 |
| キャンセル待ち連絡 | イベント発火 | 繰り上げ参加者決定時に自動送信。Confirm Templateで参加/辞退ボタンを提示 |
| オファー期限切れ | イベント発火 | 繰り上げオファーの有効期限切れ時に自動送信 |
| 対戦組み合わせ | 管理者手動 | 組み合わせサマリー画面から管理者が「LINE通知送信」を押下 |
| 参加予定リマインダー | スケジュール | 毎朝8時実行。管理者が「何日前に送信するか」を設定可能 |
| 締め切りリマインダー | スケジュール | 毎朝8時実行。管理者が「何日前に送信するか」を設定可能 |

#### LINE Postback（キャンセル待ち連絡）
- Confirm Template Messageで「参加する」「辞退する」ボタンを表示
- ユーザーがボタンをタップ → LINE Postback Webhook → バックエンドで参加/辞退処理
- 処理完了後、LINEに確認メッセージを返送
- 二重回答防止: 参加者ステータスがOFFERED以外の場合は「既に処理済みです」を返送

#### ユーザー機能
- LINE通知の有効化/無効化
- 有効化時にチャネルが割り当てられ、QRコード/URLで友だち追加
- 通知種別ごとのON/OFF切り替え（6種類）

#### 管理者機能
- チャネルの登録・無効化・有効化・強制解除
- チャネルステータス統計表示（空き/割当済/連携済/無効）
- 抽選結果・対戦組み合わせのLINE一斉送信
- スケジュール型通知の送信タイミング設定

#### チャネル回収
- 90日間ログインなし → 回収警告通知（アプリ内）
- 警告から7日経過 → チャネル回収（RECLAIMED化、チャネルをAVAILABLEに戻す）
- 毎日3時に自動実行

#### セキュリティ
- チャネルシークレット・アクセストークンはAES-256-GCMで暗号化保存
