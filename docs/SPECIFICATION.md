...
隣室予約時に、管理者のブラウザをアプリの `/api/venue-reservation-proxy/*` 経由で会場サイトへ中継し、予約申込トレイ表示から申込完了検知までを自ドメイン内で扱う。現時点ではバックエンド Controller / Service 層とフロントエンドの API クライアント / venue 判別ユーティリティまで実装済みで、`PracticeList.jsx` の画面導線接続は後続タスクで追加する。
