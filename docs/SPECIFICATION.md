...
1. `SameDayConfirmationScheduler`（毎日12:00 JST）が当日セッションの OFFERED 参加者を一括で DECLINED に変更
2. WON参加者全員にメンバーリストFlex Message（青ヘッダー）を送信
...
当日キャンセル補充フローの通知は以下の3種の独立トグルで制御される。いずれも `line_notification_preferences` テーブルで管理。

| トグル | 設定キー | デフォルト | 説明 |
|--------|---------|-----------|------|
| 参加者確定通知 | `same_day_confirmation` | true | 12:00確定時のメンバーリスト送信 |
| 当日キャンセル通知 | `same_day_cancel` | true | WON参加者へのキャンセル発生通知 |
| 空き募集通知 | `same_day_vacancy` | true | 非WON参加者への空き枠募集通知 |

**Flex Messageデザイン**

| メッセージ | ヘッダー色 | ボタン | 送信先 |
|-----------|-----------|--------|--------|
| 参加者確定（メンバーリスト） | 青 | なし | WON参加者 |
...
| `SameDayConfirmationScheduler` | 毎日12:00 JST | 当日セッションのOFFERED→DECLINED一括変更 + WON参加者にメンバーリストFlex Message送信 |
...
| 当日参加者確定 | スケジュール型（自動） | 毎日12:00 JST、当日セッションのWON参加者にメンバーリストFlex Message送信 |
