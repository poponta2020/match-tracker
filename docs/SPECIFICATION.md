...
- ロック/解除は専用 API（`PATCH /api/match-pairings/{id}/lock`・`/unlock`）で即時反映。未保存の組は `createBatch` で保存して `id` を確定してからロックする。
- 保護の挙動は結果ロックと同等に一般化する（保護判定 = `hasResult OR locked`）: 自動マッチングから対象2選手を除外し、一括保存・回戦削除でも保持する。
- 二重ブッキング防止: ロック時、同一 `(session_date, match_number)`・同一組織スコープ内で対象2選手のいずれかが別の組に含まれる場合は 409 Conflict で拒否（サーバーで「1選手1組」を担保）。
- 解除は誰でも可能（ロックした本人に限定しない）。`locked` を false に戻すのみで組は残り、通常の未ロック組（編集・削除・自動再生成の対象）に戻る。
- 表示の区別: 手動ロック=「🔒 ロック」バッジ＋全ロール向け「解除」ボタン / 結果入力済み=「結果入力済」バッジ＋ADMIN以上の「リセット」。ロック表示・操作は編集画面（PairingGenerator）のみ（PairingSummary は対象外）。
- 結果ロックとの併存: 手動ロック組に後から結果が入れば `hasResult=true` となり結果ロックとしても保護される。手動ロックを解除しても、結果が存在する限り結果ロックとして保護は継続する。
...
- 同一日内なら、対戦組み合わせを再生成しても画面ロード時に同じ札ルールが復元される（対戦テキスト部分のみ最新の対戦データに基づき再生成）
- 画面ロード時、クライアント端末ローカルタイムの「今日」と一致しない日付の保存値はまとめて削除する（過去日にアクセスしても保存はしない）
- 試合数と保存済み札ルール長の不一致時:
  - 保存値が短い（試合数が増えた）: 既存分はそのまま使用し、不足分のみ末尾追加して localStorage を上書き
  - 保存値が長い（試合数が減った）: 先頭から現セッション分のみ表示。localStorage 側は保持し、試合数が戻れば再利用可能
- localStorage パース失敗・配列でない・利用不可環境ではフォールバックして毎回ランダム生成する
- 「札を再生成」ボタン押下時は `window.confirm('現在の札ルールを上書きして再生成します。よろしいですか？')` で確認し、OK のみ再生成・上書き保存する
