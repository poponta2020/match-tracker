...
- **編集モード:** 「編集」ボタンで編集へ入る際にキャンセルスロットを「空き」として実体化する（`materializeCancelledSlots`: 片方キャンセルは当該スロットを空きに・フラグ解除、両方キャンセルの組は除去）。キャンセルした選手は参加者プール・待機リストにも現れない（アクティブ参加者でないため既存ロジックで自然に除外）。空きのまま保存した場合、未完成組は `createBatch` に送られず（`buildSaveRequests` は両選手揃った組のみ送信）、キャンセルしていない生存側の選手はアクティブ参加者として残る（データ消失なし）。
