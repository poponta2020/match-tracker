...
WON（抽選確定後の当選）または PENDING（抽選未実行・フェーズ1の自分の申込）をキャンセルするための専用ページ。カレンダー画面の「出欠登録」モーダル経由で遷移。

**フロー:**
1. キャンセル専用カレンダーが表示され、自分が WON もしくは PENDING で登録されている日がハイライト
2. 日付を選択すると、その日の自分のキャンセル可能な試合一覧が表示される（試合ごとに「当選」または「申込」のラベルが付く）
3. キャンセルしたい試合を選択（同一日内で複数選択可能）
4. キャンセル理由を選択（必須）
5. 「キャンセルする」ボタン → 確認ダイアログ（当日12:00以降の場合は「直前のキャンセルとなりますがよろしいですか？」の追加確認ダイアログを表示。補充募集の発火可否はステータス別の挙動を参照）
6. 確定でキャンセル処理実行 → 完了ダイアログ → カレンダー画面に戻る

**ステータス別の挙動:**
- WON のキャンセル: 従来どおり、当日12:00以降は当日補充フロー（全体募集＋先着ボタン方式）、それ以前はキャンセル待ち繰り上げフローが発火する。
- PENDING のキャンセル: 抽選未実行（フェーズ1）のため、`status=CANCELLED, dirty=true` のみ設定。当日12:00以降であっても補充募集・繰り上げ通知は発火しない（フェーズ1には WAITLISTED が存在せず、当日補充も抽選確定後の概念のため）。`dirty=true` により伝助同期で当該プレイヤーは × として書き戻される。
