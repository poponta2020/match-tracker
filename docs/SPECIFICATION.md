...
伝助インポートはフェーズ別に動作する:
- **フェーズ1（抽選未確定）**: ○の人のみPENDINGとして取り込み。not-○の dirty=false レコードは削除（ただしCANCELLED/DECLINED/WAITLIST_DECLINEDなどキャンセル履歴は保持）。書き戻しなし。MONTHLY型では締切日時にかかわらず抽選未確定の間は本フェーズで動作する
...
> 旧「フェーズ2（締切後・抽選確定前のスキップ）」は廃止された（Issue #616）。締切なしモード（`lottery_deadline_days_before = -1`）でも、抽選確定の有無のみでフェーズが切り替わる。
