...
| 位置 | 要素 | 書式 | スタイル |
|---|---|---|---|
| 行頭 | 日付 | `M/D`（例: `5/23`） | `text-xs text-[#9ca3af]`、`flex-shrink-0` |
| 中央 | 対戦相手名 | 選手名 | `flex-1 min-w-0 truncate`、リンク化時はテーマ色 `#4a6b5a` |
| 名前の右 | 会場・試合番号 | `会場名 N試合目`（例: `あかなら・すずらん 3試合目`） | `text-xs text-[#9ca3af]`、最大幅 40%、`truncate` |
...
|---|---|---|---|
| `/` | ホーム | ALL | ダッシュボード |
| `/matches` | 対戦一覧 | ALL | 自分・他選手の対戦履歴一覧。対戦相手名タップで相手の対戦一覧へ、メモアイコンタップで対戦詳細へ遷移（詳細仕様は3.4.6参照） |
