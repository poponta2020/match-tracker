 ...
 *
 * <p>Render free はシェル（{@code jcmd}）が使えず NMT サマリを取り出せないため、
 * {@link ManagementFactory} の MXBean 群と Linux の {@code /proc/self/status} から
 * プログラム的に収集してアプリログへ残す方式を採る（既存の Render ログ取得ツールで読み出せる）。</p>
 *
