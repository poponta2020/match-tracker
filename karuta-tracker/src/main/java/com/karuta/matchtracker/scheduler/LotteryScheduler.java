 ...
 *
 * 【現在は無効化】
 * 自動抽選は現在停止中であり、運用は管理者が `/admin/lottery` 画面から手動で実行する。
 * `@Scheduled` および `@EventListener` がコメントアウトされており、Spring がメソッドを
 * 自動起動することはない。将来的に再有効化する場合は、締切タイミング・リトライ戦略・
 * 失敗時通知の運用設計を別途検討した上でアノテーションを戻すこと。
 * 詳細は仕様書 §3.7.1 / §3.7.7 を参照。
 */
@Slf4j
@Component
